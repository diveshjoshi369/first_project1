...
# SQL setup
con = sqlite3.connect("final_database")
cur = con.cursor()
...
# Analyze Scores
def analyze_scores():
    df = pd.read_sql_query("SELECT * FROM final_database", con)
    ...
    while True:
        ...
        if choice == '1':
            ...
            df = pd.read_sql_query(
                "SELECT * FROM final_database WHERE Name = ? AND Class = ?",
...
def view_scoreboard():
    df = pd.read_sql_query("SELECT * FROM final_database ORDER BY Score DESC", con).head(10)
    slow_print("\n--- SCOREBOARD ---")
    if df.empty:
...
def view_statistics():
    ...
    while True:
        ...
        if choice == '1':
            ...
            df_temp = pd.read_sql_query("SELECT Date FROM final_database", con)
            if df_temp.empty:
                slow_print("No game data available yet.")
                continue
            ...
            df_temp = pd.read_sql_query("SELECT Date, Score FROM final_database", con)
            if df_temp.empty:
            ...
            df_temp = pd.read_sql_query("SELECT Class, Score FROM final_database", con)
            if df_temp.empty:
                slow_print("No records found to analyze.")
                continue
            ...
            df_temp = pd.read_sql_query("SELECT Score, Level_Cleared FROM final_database", con)
            if df_temp.empty:
                slow_print("No records found to analyze.")
                continue
            ...
            df_temp = pd.read_sql_query("SELECT Score FROM final_database", con)
            if df_temp.empty:
                slow_print("No records found to analyze.")
                continue
